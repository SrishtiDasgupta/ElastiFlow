...
# Scheduler selection
if ALGO == "fcfs":
    from scheduler.fcfs_optimized import FCFS_Optimized as SchedCls
    sched_kwargs = {"sort_key": sort_key_str}
elif ALGO == "edf":
    from scheduler.earliest_deadline_edf import EarliestDeadlineEDF as SchedCls
    sched_kwargs = {"sort_key": sort_key_str}
elif ALGO == "heft":
    ...
    from scheduler.heft_heft_req import HEFT_HEFT_REQ as SchedCls
    sched_kwargs = {}
elif ALGO == "rank":
    # Md.Rank in the appendix uses PriorityPriority with the
    # (BUDGET_FACTOR, DEADLINE_FACTOR) pair patched above.
    from scheduler.priority_priority import PriorityPriority as SchedCls
    sched_kwargs = {}
