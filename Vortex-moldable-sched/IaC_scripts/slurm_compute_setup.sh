...
sudo -u ubuntu bash <<'USEREOF'
set -x
if [ -d /fsx/Vortex-mid ]; then
    cp -r /fsx/Vortex-mid ~/Vortex
    echo "Vortex codebase copied from Vortex-mid"
elif [ -d /fsx/Vortex ]; then
    cp -r /fsx/Vortex ~/Vortex
    echo "Vortex codebase copied from Vortex"
else
    echo "WARNING: No Vortex directory found on /fsx"
fi
...
" || echo "WARNING: CIFAR-10 download failed, will retry later"
else
