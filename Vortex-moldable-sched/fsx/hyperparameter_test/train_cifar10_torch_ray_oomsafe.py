...
def train_cifar10_torch(config):
    ...
    )  # freeze trunk by default
    model_name = str(
        config.get("model_name", os.environ.get("MODEL_NAME", "resnext50_32x4d"))
    )

    # DEBUG: Log which model is being used
    print(f"🔍 TRAINING WITH MODEL: {model_name}")
    print(
        f"🔍 FULL CONFIG: epochs={epochs}, lr={lr}, batch_size={batch_size}, image_size={image_size}"
    )
    ...
    train_ds = torchvision.datasets.CIFAR10(
        data_dir, train=True, download=True, transform=tfm_train
    )
    ...
    # ----- Model (generic) -----
    model = build_model(
        model_name=model_name, out_dim=out_dim, train_backbone=train_backbone
    )
