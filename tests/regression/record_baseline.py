...
11 SeisSol--TinyDA variants (N=100, seed 7) and the 5 licence policies (N=150,
seed 7), produced by the cell runners in conftest.py, which are the ones the
...
from conftest import (LICENCE_N, LICENCE_POLICIES, LICENCE_SEED, REPO, SEISSOL_N,  # noqa: E402
                      SEISSOL_SEED, licence_cell, seissol_cell, seissol_variants,
                      strip_bookkeeping)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for variant in sorted(seissol_variants()):
            t0 = time.time()
